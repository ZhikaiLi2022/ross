...
#-----------------------------------------------------------------------------------------------------------------------------
path3 = '/home/life/双星偏心率的求解/allEBselct/default_period.txt'
f = open(path3,'a')
...
for ii in range(len(IDdir)):
    ...
    pg = lc.to_periodogram( method="LS",minimum_period=0.09,maximum_period=25,oversample_factor=400) 
    ...
    f.write("{: <20}".format(str(pg.period_at_max_power.value)))
    f.write("\n")
    ...
    plt.savefig("/home/life/双星偏心率的求解/allEBselct/e_data_picture/period_picture/"+lc.label+".png")
    plt.close('all')
f.close()
